...
const en=chr(13)+chr(10);                                   {enter}
      ...
      pg='<p class=glowka>'+en+sp;                              {akapit g≥Ûwka}
      pd='<p class=dodatek>'+en+sp;                             {akapit dodatek}

      lp='<a class=firma target=okno href="http://"';           {link poczπtek}
      ls='>';                                                   {link úrodek}
...
dlr:=strtoint(editdlr.Text);    {d≥ugoúÊ rozdzia≥u}
bld:=strtoint(editbld.Text);    {iloúÊ paragrafÛw bold}
itl:=strtoint(edititl.Text);    {iloúÊ paragrafÛw italic}
...
if ioresult<>0 then begin showmessage('B≥πd pliku <'+sr+nazwa+ewe+'>.'); edit1.setfocus; exit; end;
...
begin
 ...
 for i:=1 to 5 do
  if rys[i] and (par=poa[i]+1) then
   ...
   if zro[i] then writeln(plikwy,'<P class=zrodlo>èrÛd≥o: opracowanie w≥asne</P>');
